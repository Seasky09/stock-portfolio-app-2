...
export default function App() {
  ...
  async function saveManualPrice(ticker, value) {
    ...
    const { error: upsertError } = await supabase
      .from("portfolio_prices")
      .upsert({ user_id: session.user.id, ticker, price }, { onConflict: "user_id,ticker" });
            ...
            {activeTab === "realized" && (
              <div className="card">
                ...
                {summary.realizedRows.length === 0 ? (
                  <div className="muted">아직 매도 거래가 없습니다.</div>
                ) : (
                  <div className="tableWrap">
                    <table>
                      <thead>
                        <tr>
                          ...
                          <th>종목명</th>
                          <th className="right">수량</th>
                          <th className="right">매도단가</th>
                          <th className="right">평균매입단가</th>
                          <th className="right">실현손익</th>
                          <th className="right">수익률</th>
                        </tr>
                      </thead>
                      <tbody>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="formGrid">
              ...
              <div>
                ...
                <div className="row gap8">
                  <button
                    className={`btn ${form.type === "buy" ? "primary" : "secondary"}`}
                    onClick={() => setForm((prev) => ({ ...prev, type: "buy" }))}
                  >
                    매수
                  </button>
                  <button
                    className={`btn ${form.type === "sell" ? "primary" : "secondary"}`}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setForm((prev) => ({ ...prev, stock_name: e.target.value }))}
                  placeholder="예: 삼성전자"
                />
              </div>
